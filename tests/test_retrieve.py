...
def test_retrieve_relevant_chunks_returns_requested_count():
    chunks = retrieve.retrieve_relevant_chunks("What is the refund policy?", k=2)
    assert len(chunks) == 2


def test_retrieve_relevant_chunks_are_relevant_to_the_query():
...
def test_retrieve_relevant_chunks_have_citation_metadata():
